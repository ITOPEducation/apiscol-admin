...
class ResourceImportView extends AbstractView implements IView {
	...
	private function getImportMetadataArea() {
		...
		$area = '<div class="add-metadata-input-container"><form id="import-metadata" enctype="multipart/form-data"	action="' . $action . '" method="POST"><input id="metadata_upload" type="file" name="import-metadata" /><input id="metadata_submit" type="submit" value="Créez une nouvelle entrée en important un fichier scoLOMfr" />' . '</form>	<div class="progress">	<div class="bar"></div><div class="percent">0%</div></div></div>';
		return $area;
	}
	...
	private function getContentLinkSuggestionArea($link, $tag = null) {
		...
		if (is_null ( $tag ))
			...
			$area = '<div class="content-suggestion-wrapper ui-state-focus"><h3>Ce contenu a été trouvé dans la balise : ' . $tag . '</h3>';
		$area .= '<div>' . $link . '</div>';
		$area .= '<form action="' . $this->prefix . '/add/import' . '" method="POST" class="register-url">';
