		...
		foreach ( self::$_BUFFER as $key => $value ) {
			...
			switch ($type) {
				...
				case "action" :
					...
					break;
				case "panel" :
					$valid = isset ( self::$_BUFFER ["page"] ) && array_key_exists ( self::$_BUFFER ["page"], self::$siteMap ) == 1 && isset ( self::$_BUFFER ["action"] ) && is_array ( self::$siteMap [self::$_BUFFER ["page"]] ) == 1 && array_key_exists ( self::$_BUFFER ["action"], self::$siteMap [self::$_BUFFER ["page"]] ) == 1 && in_array ( $value, self::$siteMap [self::$_BUFFER ["page"]] [self::$_BUFFER ["action"]] ) == 1;
					break;
				case "resource-type" :
					$valid = in_array ( $value, array (
							"asset",
