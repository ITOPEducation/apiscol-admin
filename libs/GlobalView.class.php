...
class GlobalView {
	...
	private function createMenu() {
		...
		$this->menu->addItem ( 'MAINTENANCE', "maintenance", $_SESSION ['page'], false );
	...
	public function getAuthorizationArea() {
		if ($this->mainController->getAuthorizationStatus () == AuthorizationStatus::NOT_CONNECTED)
			return '<span class="status connexion">{CONNEXION}</span>' . '<div class="connexion-box ui-state-highlight ui-corner-all">' . '<form action="' . $_SERVER ['REQUEST_URI'] . '" method="POST">' . '<input type="text" name="login" id="login" placeholder="Login" />' . '<input type="password" name="password" id="password" placeholder="{PASSWORD}" />' . '<input type="submit" value="{SUBMIT}" />' . '</form>' . '<a href="https://cas.crdp.ac-versailles.fr/cas/login?service=http%3A%2F%2Fpas/encore/implémenté">Connexion CAS</a>' . '</div>';
		else
			return '<span class="status disconnect"><span class="user-login">' . $this->mainController->getUserLogin () . '</span><form action="' . $_SERVER ['REQUEST_URI'] . '" method="POST">' . '<input type="hidden" name="disconnect" id="disconnect" value="disconnect" />' . '<span class="disconnect-wrapper"><input type="submit" value="{DISCONNECT}"></span>' . '</form>' . '</span>';
	}
}
